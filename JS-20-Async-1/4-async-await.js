...
const getUsers = async () =>{
    ...
    // throw ile kendi hata mesajini olusturabilirsin. Fakat try-catch kullanmazsan hata mesajindan sonraki kod satirlari calismaz. try-catch ile bunu gerceklestirebilirsiniz
    throw new Error(`Url de hata var res ${res.status}`)
    }
   ...
   const veri = await res.json()
    ekranaBastir(veri)
        ...
        console.log(error);
        console.log("try-catch ile kod devam ediyor");
    }
...
const ekranaBastir = (veri) => {
    ...
    veri.forEach((user) => {
    girlsDiv.innerHTML += ` <h2>${user.show.name} </h2>
