...
// console.log(document.querySelector("input").value);
 ...
 let yeniDizi= showList.meals.filter((a)=> a.strMeal.toLowerCase().includes(e.target.value.toLowerCase()) )
...
ülkeler.forEach((ülke) => {
  ülke.onclick = () => {
    fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?a=${ülke.id}`).then((res)=>res.json()).then((data)=>ekranaBastir(data.meals))
  }
})